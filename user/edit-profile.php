...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Phone validation: must start with 03 and be exactly 11 digits
    if(!empty($phone) && !preg_match('/^03[0-9]{9}$/', $phone)) {
        $error = 'Phone number must start with 03 and be exactly 11 digits (e.g., 03123456789)';
    } else {
        mysqli_query($conn, "UPDATE users SET full_name = '$full_name', phone = '$phone' WHERE id = '$user_id'");
...
<head>
    ...
    <title>Edit Profile - UHD-Wears</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
            ...
            .edit-card .card-body { padding: 20px; }
            .btn { width: 100%; margin-bottom: 10px; }
            .d-flex.gap-2 { flex-direction: column; gap: 10px !important; }
            .edit-card .card-header h4 { text-align: center; }
        }
        
        @media (max-width: 576px) {
            .dashboard-wrapper { padding: 60px 0 15px; }
            .edit-card .card-body { padding: 15px; }
                ...
                <div class="sidebar-dash" id="sidebar">
                    <div class="text-center mb-4">
                        <div class="avatar-circle" style="width: 80px; height: 80px; background: linear-gradient(135deg, #FFD700, #FFC107); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
                            <i class="fas fa-user-edit" style="font-size: 38px; color: #000;"></i>
                        </div>
                    ...
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link" href="orders.php"><i class="fas fa-shopping-bag"></i> My Orders</a>
                        <a class="nav-link" href="track-order.php"><i class="fas fa-truck"></i> Track Order</a>
                        ...
                        <a class="nav-link" href="edit-address.php"><i class="fas fa-map-marker-alt"></i> Edit Address</a>
                        <hr style="border-color: rgba(255,255,255,0.1);">
                    ...
                    <div class="card-body">
                        <form method="POST" id="editProfileForm">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="tel" name="phone" id="phone" class="form-control" value="<?php echo htmlspecialchars($user['phone']); ?>" placeholder="03XXXXXXXXX">
                                <small class="text-muted">
                                    <i class="fas fa-info-circle me-1"></i> Must start with 03 and be exactly 11 digits (e.g., 03123456789)
                                </small>
                            </div>
...
function showSweetAlert(icon, title, message, redirect = null) {
    Swal.fire({
        icon: icon,
        title: title,
        html: message,
...
// Form submission with SweetAlert (NO browser alerts)
document.getElementById('editProfileForm').addEventListener('submit', function(e) {
    ...
    if(fullName === '') {
        showSweetAlert('error', 'Missing Information', 'Please enter your full name!');
        return false;
    }
    ...
    if(phone !== '') {
        ...
        if(!phoneRegex.test(phone)) {
            showSweetAlert('error', 'Invalid Phone Number', 'Phone number must start with 03 and be exactly 11 digits!<br><small>Example: 03123456789</small>');
            return false;
        }
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            } else if(response.includes('Phone number must start with 03')) {
                showSweetAlert('error', 'Invalid Phone Number', 'Phone number must start with 03 and be exactly 11 digits!');
            } else {
                showSweetAlert('error', 'Update Failed', 'Something went wrong. Please try again.');
            }
        },
        error: function() {
            showSweetAlert('error', 'Error', 'Something went wrong. Please check your connection and try again.');
        }
    });
});
